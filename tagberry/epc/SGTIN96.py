...
from tagberry.epcerrors import InvalidSerialNumber
from tagberry.epcerrors.EncodingException import EncodingException
from tagberry.gs1.GTIN import GTIN
...
class SGTIN96(EPCNumber):
    ...
    def __init__(self, startSerialNumber=0, numOfSerialNumbers=0, fixedSerialNumberLength=0):
        ...
        """
        super().__init__(startSerialNumber, numOfSerialNumbers, fixedSerialNumberLength)
        self._loadFields()
    ...
    def encode(self, *args, **kwargs):
        ...
        self._packStringFormat = 'uint:8, uint:3, uint:3, uint:%s, uint:%s, uint:38' % (
        companyPrefixField.getBitLength(), itemReferenceField.getBitLength())
        # Pack the bitstring
        ...
        if self.fixedSerialNumberLength > 0:
            ...
            if int(serial_number) < lower_bound or int(serial_number) > upper_bound:
                raise InvalidSerialNumber(
                    "A fixed-length serial number of {0} digits must be a value between {1} and {2}. The value supplied was {3}",
                    self.fixedSerialNumberLength, lower_bound, upper_bound, serial_number)
        else:
            if int(serial_number) > int(math.pow(2, 38) - 1):
                raise InvalidSerialNumber(
                    "The serial number is out of range. The serial number must be less than or equal to {0}. The value supplied was {1}",
                    upper_bound, serial_number)
    ...
    def setFieldValue(self, fieldName, val, ItemReference=False):
        ...
        if not ItemReference and fieldName == 'ItemReference':
            raise FieldValueException(
                'To change the ItemReference value after construction use the setItemReference and setIndicatorDigit helper functions.  Otherwise, set ItemReference to True.')
    ...
    def updateBitString(self):
        self._packStringFormat = 'uint:8, uint:3, uint:3, uint:%s, uint:%s, uint:38' % (
        self.getField("CompanyPrefix").getBitLength(), self.getField("ItemReference").getBitLength())
        # Pack the bitstring
        bsp = bitstring.pack(self._packStringFormat,
                             self.getFieldValue("Header"),
                             self.getFieldValue("Filter"),
    ...
    def fromURI(self, uri):
        ...
        filter_value = 0
        serialNumber = s[2]
        self.encode(companyPrefix=companyPrefix, indicatorDigit=indicatorDigit, itemReference=itemReference,
                    filter=filter_value, serialNumber=int(serialNumber))
        return self
    ...
    def fromTagUri(self, tagUri):
        ...
        serialNumber = s[4]
        self.encode(companyPrefix=companyPrefix, indicatorDigit=indicatorDigit, itemReference=itemReference,
                    filter=filter_val, serialNumber=int(serialNumber))
        return self
    ...
    def toTagURI(self):
        ...
        epcUri = "urn:epc:tag:sgtin-96:%s.%s.%s.%s" % (
            self.getFieldValue("Filter"), self.getFieldValue("CompanyPrefix"),
    ...
    def toEPCUri(self):
        ...
        if len(self.getFieldValue("CompanyPrefix")) + len(self.getFieldValue("ItemReference")) != 13:
            raise EncodingException("The Length of the CompanyPrefix and the ItemReference must equal 13")
        epcUri = "urn:epc:id:sgtin:%s.%s.%s" % (
            self.getFieldValue("CompanyPrefix"), self.getFieldValue("ItemReference"),
            int(self.getFieldValue("SerialNumber")))
        return epcUri
    ...
    def toGS1(self, useParenthesesAroundAIs=True):
        ...
        gtin.encode(self.getFieldValue("ItemReference")[0:1], self.getFieldValue("ItemReference")[1:],
    ...
    def decodeFromBinary(self, binary):
        ...
        self.setFieldValue("CompanyPrefix", companyPrefix)
        itemReference = binary[self.getField("ItemReference").getOffset():self.getField(
            "ItemReference").getOffset() + self.getField("ItemReference").getBitLength()]
        ...
        self._ignoreUpdate = False
        serialNumber = binary[self.getField("SerialNumber").getOffset():self.getField(
            "SerialNumber").getOffset() + self.getField("SerialNumber").getBitLength()]
    ...
    def _loadFields(self):
        ...
        companyPrefix = Field(fieldName="CompanyPrefix", offset=14, bitLength=24, ordinal=4, fieldValue=0,
                              digitLength=6, isPadded=True)
        self._fieldDictionary["CompanyPrefix"] = companyPrefix
        itemReference = Field(fieldName="ItemReference", offset=38, bitLength=24, ordinal=5, fieldValue=0,
                              digitLength=6)
