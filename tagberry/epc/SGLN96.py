...
class SGLN96(EPCNumber):
    ...
    def encode(self, companyPrefix, indicatorDigit, locationReference, filter_value, extension=0):
        ...
        self._packStringFormat = 'uint:8, uint:3, uint:3, uint:%s, uint:%s, uint:41' % (
        companyPrefixField.getBitLength(), locationReferenceField.getBitLength())
        # Pack the bitstring
    ...
    def _updateBitString(self):
        # Set the PackString Format
        self._packStringFormat = 'uint:8, uint:3, uint:3, uint:%s, uint:%s, uint:41' % (
        self.getField("CompanyPrefix").getBitLength(), self.getField("LocationReference").getBitLength())
        # Pack the bitstring
        bsp = bitstring.pack(self._packStringFormat,
                             self.getFieldValue("Header"),
                             self.getFieldValue("Filter"),
    ...
    def toEPCTagUri(self):
        ...
        '''
        epcUri = "urn:tagpy:tag:sgln-96:%s.%s.%s.%s" % (
        self.getFieldValue("Filter"), self.getFieldValue("CompanyPrefix"), self.getFieldValue("LocationReference"),
        self.getFieldValue("Extension"))
        return epcUri
    ...
    def toEPCUri(self):
        '''Returns the SGLN-96 in an EPC URI Representation'''
        epcUri = "urn:tagpy:id:sgln:%s.%s.%s" % (
        self.getFieldValue("CompanyPrefix"), self.getFieldValue("LocationReference"), self.getFieldValue("Extension"))
        return epcUri
    ...
    def decodeFromBinary(self, binary):
        ...
        # Filter
        filter = binary[self.getField("Filter").getOffset():self.getField("Filter").getOffset() + self.getField(
            "Filter").getBitLength()]
        self.setFieldValue("Filter", int(filter, 2))
        partitionValue = binary[
                         self.getField("Partition").getOffset():self.getField("Partition").getOffset() + self.getField(
                             "Partition").getBitLength()]
        ...
        companyPrefix = binary[self.getField("CompanyPrefix").getOffset():self.getField(
            "CompanyPrefix").getOffset() + self.getField("CompanyPrefix").getBitLength()]
        companyPrefix = str(int(companyPrefix, 2)).zfill(companyPrefixDigitLength)
        self.setFieldValue("CompanyPrefix", companyPrefix)
        locationReference = binary[self.getField("LocationReference").getOffset():self.getField(
            "LocationReference").getOffset() + self.getField("LocationReference").getBitLength()]
        ...
        extension = binary[
                    self.getField("Extension").getOffset():self.getField("Extension").getOffset() + self.getField(
                        "Extension").getBitLength()]
    ...
    def _loadFields(self):
        ...
        locationReference = Field(fieldName="LocationReference", offset=21, bitLength=24, ordinal=5, fieldValue=0,
                                  digitLength=6)
