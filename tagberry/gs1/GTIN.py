...
from tagberry.gs1 import gtin_patterns
...
class GTIN(GS1Number):
    ...
    def toGS1(self, useParenthesesAroundAIs=False):
        if useParenthesesAroundAIs == True:
            if self.getUseFixedSerialNumber() is True:
    ...
    def toURN(self):
        return 'urn:tagpy:id:sgtin:{0}.{1}{2}.{3}'.format(self._companyPrefix, self._indicatorDigit,
                                                          self._itemReference, int(self._serialNumber))
